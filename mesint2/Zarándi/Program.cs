...
int[] gepek = new int[] { 5, 10 };
int[] munkak = new int[] { 20, 50 };
...
static void futat(int gepek, int job)
{
    ...
    Console.WriteLine($"gépek száma:{gepek}, munkák száma {job}");
    int a = p.GetLength(0) * p.GetLength(1) *100 ;
    Console.WriteLine("palmer sor: ");
    foreach (var i in palmer(p))
    {
        Console.Write($"{i},");
    }
    Console.WriteLine($"palmer sor eredménye: {eredmeny(p, palmer(p))}");
    szimhut(palmer(p), p, a);


}
static int[] palmer(int[,] p) {
    Dictionary<int, int> valami = new Dictionary<int, int>();
    int m = p.GetLength(0);
    int[] sor = new int[p.GetLength(0)];
    for (int i = 0; i < p.GetLength(0); i++)
    {
        int sum = 0;
        for (int j = 0; j < p.GetLength(1); j++)
    ...
    int[] keys = new int[sor.Length];
    int count = 0;
    foreach (KeyValuePair<int, int> i in valami.OrderBy(x => x.Value))
    {
        keys[count] = i.Key;
...
static void szimhut(int[] sorrrend, int[,] p, int iter)
{
    ...
    Console.WriteLine($"A legjobb megoldás értéke: {ered1}");
